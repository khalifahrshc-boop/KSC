  ...
  return (
    <div id="dashboard-module-root" className="relative min-h-screen dashboard-root-container">
                ...
                <div key={upd.id} className={`bg-gray-50/50 border p-4 rounded-xl space-y-3 hover:bg-white hover:border-blue-200 hover:shadow-md transition-all group relative ${isDeleteMode ? 'border-red-200 bg-red-50/30' : 'border-gray-100'}`}>
                  ...
                  <div className="flex flex-col gap-2 pt-2.5 border-t border-gray-100">
                    <div className="flex items-center gap-2 text-[10px] text-gray-500 font-bold">
                      ...
                      <span className="ml-auto text-[9px] bg-gray-100 px-1.5 py-0.5 rounded text-gray-400 border border-gray-200">
                        {upd.numberOfWorkers} {isRtl ? 'عمال' : 'workers'}
                      </span>
                    </div>
                    {upd.workerNames && upd.workerNames.length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {upd.workerNames.map((name, idx) => (
                          <span key={idx} className="text-[8px] font-bold bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded border border-amber-100 flex items-center gap-1">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col justify-between space-y-4">
            <div className="flex justify-between items-center border-b border-gray-50 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-black text-[#040957] font-sans">
                  {isRtl ? 'المذكرات والملاحظات السريعة' : 'Quick Memos & Notes'}
                </h2>
              </div>
              <button
                onClick={() => setIsQuickNoteOpen(true)}
                className="text-[10px] bg-indigo-50 text-indigo-600 hover:bg-indigo-100 font-black px-2.5 py-1.5 rounded-lg transition uppercase flex items-center gap-1 cursor-pointer"
              >
                <span>+</span>
                <span>{isRtl ? 'إضافة' : 'Add Note'}</span>
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto max-h-[250px] space-y-3 pr-1 scrollbar-thin scrollbar-thumb-gray-200">
              {quickNotes.map((note) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-[9px] text-gray-400 font-bold border-t border-gray-100 pt-2">
                    <UserIcon className="w-3 h-3 text-indigo-400" />
                    <span className="truncate text-gray-500">{note.userName}</span>
                    <span className="ml-auto bg-gray-100 px-1.5 py-0.5 rounded text-gray-400 border border-gray-200 font-mono">
                      {note.date}
                    </span>
                  </div>
                </div>
              ...
              {quickNotes.length === 0 && (
                <div className="h-32 flex flex-col items-center justify-center text-center p-6 space-y-2 opacity-40">
                  <FileText className="w-10 h-10 text-gray-300 mx-auto" />
                  <p className="text-xs font-bold text-gray-400 italic">
                    {isRtl ? 'لا توجد مذكرات سريعة بعد...' : 'No quick memos recorded yet...'}
                  </p>
                </div>
              )}
            </div>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        onClick={() => setIsQuickNoteOpen(true)}
        className="fixed bottom-6 right-6 rtl:left-6 rtl:right-auto z-[99] bg-[#0080FF] hover:bg-blue-600 text-white p-4 rounded-full shadow-2xl flex items-center justify-center gap-2 group border-2 border-white cursor-pointer"
      ...
      >
        ...
        <span className="max-w-0 overflow-hidden group-hover:max-w-xs transition-all duration-300 text-xs font-black uppercase tracking-wider whitespace-nowrap">
        ...
        {isQuickNoteOpen && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="bg-white rounded-2xl shadow-2xl max-w-md w-full border border-gray-100 overflow-hidden font-sans"
            >
              {/* Header */}
              <div className="bg-[#040957] text-white p-5 flex justify-between items-center">
                <div className="flex items-center gap-2">
              ...
              {/* Form */}
              <form onSubmit={handleSaveNoteSubmit} className="p-6 space-y-4">
                {/* Metadata tags */}
                <div className="bg-slate-50 p-3 rounded-xl border border-gray-100 grid grid-cols-2 gap-3 text-[10px] font-bold">
                  <div>
                    <span className="block text-gray-400 uppercase tracking-wider text-[8px] mb-0.5">{isRtl ? 'المستخدم النشط' : 'User Identity'}</span>
                    <span className="text-[#040957]">{currentUser.name}</span>
                  </div>
                  <div>
                    <span className="block text-gray-400 uppercase tracking-wider text-[8px] mb-0.5">{isRtl ? 'التاريخ الفعلي' : 'Date stamp'}</span>
                    <span className="text-[#040957] font-mono">{new Date().toISOString().split('T')[0]}</span>
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-wider">
                    {isRtl ? 'محتوى المذكرة / التحديث السريع' : 'Memo content / rapid-fire status update'}
                  </label>
                  <textarea
                    required
                    rows={4}
                    ...
                    placeholder={isRtl ? 'اكتب مذكرتك أو تحديثك الميداني السريع هنا...' : 'Type your rapid-fire status update or internal memo...'}
                    className="w-full border border-gray-200 rounded-xl p-3 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 text-gray-700 font-medium"
                  />
                  <div className="text-[9px] text-gray-400 font-bold text-right">
                    {quickNoteContent.length} {isRtl ? 'حرفاً' : 'characters'}
                  </div>
                </div>
                ...
                <div className="flex gap-3 justify-end pt-2">
                  ...
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-xl transition font-black text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-pointer ${isSavingNote || !quickNoteContent.trim() ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-[#0080FF] hover:bg-blue-600 text-white shadow-lg shadow-blue-100'}`}
                  >
                    {isSavingNote ? (
                    ...
                    <span>{isRtl ? 'حفظ المذكرة' : 'Save Memo'}</span>
                  </button>
                </div>
              </form>
            </motion.div>
